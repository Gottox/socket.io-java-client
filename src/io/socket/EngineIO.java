...
class EngineIO implements IOCallback {
	...
	private static HashMap<String, List<EngineIO>> connections = new HashMap<String, List<EngineIO>>();
	...
	private class ConnectThread extends Thread {
		...
		@Override
		public void run() {
			if (EngineIO.this.getState() == STATE_INIT)
				handshake();
			connectTransport();
		}
	...
	static public EngineIO register(String origin, SocketIO socket) {
		...
		if (list == null) {
			list = new LinkedList<EngineIO>();
			connections.put(origin, list);
		} else {
			for (EngineIO connection : list) {
				if (connection.register(socket))
					return connection;
			}
		}

		EngineIO connection = new EngineIO(origin, socket);
		list.add(connection);
		return connection;
	}
	...
	private void handshake() {
		...
		try {
			...
			url = new URL(EngineIO.this.url.toString() + SOCKET_IO_1);
	 ...
	 *            the socket
	 */
	private EngineIO(String url, SocketIO socket) {
		try {
